...
CellPtr procNil(const Cells& args) {
    ...
    return args[0]->type == List && args[0]->list.empty() ? std::make_shared<Cell>(Symbol, "t") : std::make_shared<Cell>(Symbol, "nil");
}
...
CellPtr procSymbol(const Cells& args) {
    ...
    CellPtr arg = args[0];
    return arg->type == Symbol ? std::make_shared<Cell>(Symbol, "t") : std::make_shared<Cell>(Symbol, "nil");
}
...
CellPtr atom(const std::string& token) {
    ...
    if (isNumber(token)) {
        return std::make_shared<Cell>(Number, token);
    }
